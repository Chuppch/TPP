...
修改核心搜索时，必须对照本文件中的算法执行链路、模块责任和以下论文一致性约定：
...
- Constructive 中多个市场同时满足 `w_i` 和 `a_i` 条件时，选择市场编号最小者。
- Repair 中 `G_iR` 和缺失商品覆盖数仍完全并列时，选择市场编号最小者。
- 多个插入位置的旅行成本增量相同时，选择路线中最靠前的位置。
- 多个邻域解总成本相同时保留当前解，只有严格降低目标值才算改进。
- 浮点比较统一使用 `EPSILON = 1e-9`。
- Destroy 使用由 `ILSConfig.seed` 初始化的独立随机数生成器。
- 如果论文补充材料给出不同规则，以论文材料为准，并同步更新代码、测试、README 和本文件。
...
- 论文对应关系、并列规则或默认参数发生变化时，同步更新 `README.md`、本文件和相关测试。
